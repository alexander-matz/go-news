...
var (
	...
	appDbUri  = app.Flag("db-conn", "Database connection string.").Short('c').Default("sqlite3://./data.etilqs").String()
	...
	addDefFeeds    = add.Command("deffeeds", "Add default feeds.")
	...
	initializeDefault = initialize.Command("defaults", "Initialize the database with defaults.")
	initializeNew     = initialize.Command("new", "Initialize experimental sql database.")
...
func cmdAddDefaultFeeds() error {
	...
	feeds := []struct {
		...
		{"bbce", "https://feeds.bbci.co.uk/news/world/europe/rss.xml"},
		{"csm", "https://rss.csmonitor.com/feeds/csm"},
		{"alj", "https://www.aljazeera.com/xml/rss/all.xml"},
		{"ecoc", "https://www.economist.com/sections/culture/rss.xml"},
		{"ecoi", "https://www.economist.com/sections/international/rss.xml"},
	}
	var feed db.Feed
	nerr := 0
	...
	if nerr > 0 {
		return errors.New(fmt.Sprintf("Failed to add %d feeds", nerr))
	} else {
		return nil
	}
}
...
func main() {
	...
	case "add deffeeds":
		funclet = func() error { return cmdAddDefaultFeeds() }
	case "add request":
